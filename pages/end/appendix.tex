...
\centering
  \Rotatebox{90}{
    \begin{tabular}{@{\extracolsep{\fill}}lllccccc} 
    \toprule
 message& & & & & session& users&\\ 
 \midrule
\lstinline+GET /+ &\lstinline+GET /:id+ &\lstinline+POST / +&\lstinline+DELETE /:id+ &  \lstinline+PUT /:id+ &  \lstinline+GET /+&  \lstinline+GET /+& \lstinline+GET /:id+ \\ 
\lstinline+/messages/1+ & \lstinline+/messages/2+ & \lstinline+/messages/2222+& \lstinline+/messages/2+ & \lstinline+/messages/1+ & \lstinline+/session+ & \lstinline+/users+ & \lstinline+/users/2+\\
            \bottomrule
    \end{tabular}
