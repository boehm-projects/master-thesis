...
\begin{figure}[h]
    ...
    \begin{tikzpicture}[stack/.style={rectangle split, rectangle split parts=#1,rectangle split horizontal,draw, anchor=center}]
        ...
        \node (listen) [box, yshift = 1cm] {listen};

        \begin{scope}
                \node (use) [box, left = of listen, xshift = -3cm ]{use};      
        \node (middleware) [box, below = of use, xshift = 2cm] {middlware};
        \node (router) [box, left = of middleware] {router};
        ...
        \node (stack)[stack=5, below = of middleware, xshift = -2cm,fill=orange!20]  {
            ...
            \nodepart{two} route
            \nodepart{three} route
            \nodepart{four} middleware
            \nodepart{five} middleware
            };
        \node (handle) [box, below = of listen] {handleStack};
        %
        ...
        \draw[->, thick] (stack.south east)  -- ++ (0,-0.2) -- ++(-7.5, 0)   node[midway, below] {process request};

        \draw[lmugreen,thick,dotted] ($(listen.north west)+(-1.2,0.9)$)  rectangle ($(stack.south east)+(6,-0.9)$)node[box]{request handling};
        \draw[red,thick,dotted] ($(use.north west)+(-2.4,0.9)$)  rectangle ($(stack.south east)+(0.2,-0.9)$)node[box]{stack creation};        
